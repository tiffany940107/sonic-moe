...
def build_report(suites: list[dict], baseline_label: str, output: Path) -> str:
    ...
    lines = [
        ...
        (
            "The aligned Sonic versus FlashInfer old/PR #4660 table is in "
            "[FLASHINFER_VS_SONIC.md](FLASHINFER_VS_SONIC.md). The separate four-GPU "
            "EP4 campaign is documented in [ep4/README.md](ep4/README.md)."
        ),
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "This is a separate customer-shape campaign: global tokens 16,384, "
                "4,096/rank, top-k 24, hidden 2,560, post-SwiGLU intermediate 1,024, "
                "768 global experts, 192 local experts/rank, and world size 4. Values "
                "are the median of three restart P50 four-rank critical-path CUDA-event "
                "latencies."
            ),
            "",
            ...
            "",
            (
                "FlashInfer and Sonic use external deduplicated Torch/NCCL EP wrappers "
                "around local grouped kernels; MegaMoE uses its own fused multi-rank "
                "runtime. FlashInfer is FP8 with FP32 groupwise scales, whereas Sonic "
                "and MegaMoE use MXFP8 E4M3 plus E8M0 K32 scales. This is a full-data-"
                "path comparison, not a bit-equivalent kernel A/B."
            ),
            "",
            (
                "The public Sonic/EPLB entry points are "
                "[mxfp8-ep4-e2e.py](../../benchmarks/mxfp8-ep4-e2e.py), "
                "[run-mxfp8-ep4-suite.sh](../../benchmarks/run-mxfp8-ep4-suite.sh), "
                "and [summarize-mxfp8-ep4-suite.py]"
                "(../../benchmarks/summarize-mxfp8-ep4-suite.py). The external "
                "FlashInfer wrapper is [flashinfer-fp8-ep4-e2e.py]"
                "(../../benchmarks/flashinfer-fp8-ep4-e2e.py), with "
                "[run-flashinfer-fp8-ep4-suite.sh]"
                "(../../benchmarks/run-flashinfer-fp8-ep4-suite.sh). See the "
                "[full EP4 report](ep4/README.md) for imbalance/EPLB, migration cost, "
